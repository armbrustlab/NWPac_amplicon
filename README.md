...
*During a given session, mothur will remember the most recent .fasta and .groups files it worked on, and will then attempt to feed these directly into the next function call if no specific file paths are given. (Check out the function `get.current()`.) However, the function calls in [03_16S_process.sh](scripts/03_16S_process.sh) are written with file search patterns unique enough that the user can easily end a session and "pick up" where here or she left off the next time.*
